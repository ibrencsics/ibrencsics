...
class MeanReversionStrategy:
    ...
    def execute(self, close):
        ...
        for i in range(len(close)):
            ...
            if ((apo > self.APO_VALUE_FOR_SELL_ENTRY*stdev_factor and abs(close_price - last_sell_price) > self.MIN_PRICE_MOVE_FROM_LAST_TRADE*stdev_factor)
                    # APO above sell entry threshold, we should sell
                    or (position > 0 and (apo >= 0 or open_pnl > self.MIN_PROFIT_TO_CLOSE/stdev_factor))):
            ...
            elif ((apo < self.APO_VALUE_FOR_BUY_ENTRY*stdev_factor and abs(close_price - last_buy_price) > self.MIN_PRICE_MOVE_FROM_LAST_TRADE*stdev_factor)
            ...
            # print("close: ", close_price, "buy_some_qty: ", buy_sum_qty, "sell_sum_qty: ", sell_sum_qty,
            #       "position: ", position, "sell_sum_price_qty: ", sell_sum_price_qty,
            #       "OpenPnL: ", open_pnl, " ClosedPnL: ", closed_pnl, " TotalPnL: ", (open_pnl + closed_pnl))
        ...
        # data.to_csv("mean_reversion.csv", sep=",")
